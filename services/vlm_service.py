...
def get_vlm_analysis(prompt_text: str, image_path: str) -> dict:
    # """
    # (新版) 调用 qwen-vl-max 模型获取对图片和文本的分析。
    # 使用 OpenAI SDK 兼容模式。
    #
    # Args:
    ...
    #         model="qwen-vl-max",  # 或者使用 "qwen-vl-max-latest"
    #         messages=messages,
    #         # temperature、top_p 等参数也可以在这里设置
    #     )
    #     # 提取模型返回的核心内容
    #     content = completion.choices[0].message.content
    #     # 我们期望返回的是 JSON 字符串，所以直接返回
